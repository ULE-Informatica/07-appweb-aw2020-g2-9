<template>
  <div class="pa-7">
    ...
    <v-card v-if="peliculasFavoritas.length>0" class="d-flex flex-wrap justify-center" flat tile>
      <v-card v-for="pelicula in peliculasFavoritas" :key="pelicula.id" outline tile
    ...
    <v-alert v-else type="info">No tienes ninguna película marcada como favorita.</v-alert>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getPeliculasFavoritas(){
      console.log("cargando películas favoritas.");
      axios.get('etiqueta/favoritas',{
        ...
        console.log("películas favoritas obtenidas.");
        ...
        this.peliculasAux.forEach(pelicula => {
          axios.get('https://api.themoviedb.org/3/movie/' + pelicula.peliculaId + '?api_key=d3500b9561bcc274c208215eeec7093b&language=es-ES'
            ...
            this.peliculasFavoritas.push({id:pelicula.peliculaId, title:resp.data.title, poster_path: resp.data.poster_path});
          }).catch((error)=>{
            console.log("error al obtener una película vista: " + error);
          })
        });
      }).catch((error)=>{
        console.log("error al cargar películas favoritas: " + error);
      })
    }  
  },

  created(){
    console.log("página películas favoritas creada.");
  },

  mounted(){
    console.log("mounted página películas favoritas.");
    this.setUsuario();
