...
export default {
  ...
  methods: {
    ...
    getSeriesPendientes(){
      console.log("cargando series pendientes.");
      axios.get('etiqueta/pendientes',{
        ...
        console.log("series pendientes obtenidas.");
        ...
        this.seriesAux.forEach(serie => {
          axios.get('https://api.themoviedb.org/3/tv/' + serie.peliculaId + '?api_key=d3500b9561bcc274c208215eeec7093b&language=es-ES'
            ...
            this.seriesPendientes.push({id:serie.peliculaId, title:resp.data.title, poster_path: resp.data.poster_path});
          }).catch((error)=>{
            console.log("error al obtener una serie vista: " + error);
          })
        });
      }).catch((error)=>{
        console.log("error al cargar series pendientes: " + error);
      })
    }  
  },

  created(){
    console.log("página series pendientes creada.");
  },

  mounted(){
    console.log("mounted página series pendientes.");
    this.setUsuario();
    this.getSeriesPendientes();
  }
